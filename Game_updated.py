            ...
            for col in range(len(self.map_list)):
                if self.map_list[row][col] == SYMB_FOR_ENEMY:
                    ...
                    if current_index == exit_index:
                        ...
                        self.map_list[row][col] = SYMB_FOR_DESTROYABLE_WALL
                        wall = WallBrickDestroyable(col * CELL_SIZE['X'], row * CELL_SIZE['Y'])
                        wall.add(self.all_sprites_group)
                    current_index += 1
        ...
        if not board.stop:
            ...
            for enemy in board.enemys:
                if enemy.index in board.enemy_move:
                    ...
                    if (abs(enemy.rect.x - board.player.rect.x) < CELL_SIZE['X']
                        and abs(enemy.rect.y - board.player.rect.y) < CELL_SIZE['Y']):
                        board.end_game(win=False)
...
def support_screen():
    ...
    support_text = ['1) В этой игре вам доступны 10 локаций для прохождения',
                    ...
                    '5) Игрок (вы) — белого цвета',
                    '6) Враги — чёрного цвета',
                    ...
                    '#5 уровень персонажа равна 0.75%, λ-уровень — 0.01%. (на 10 локации)',
